...
export function detect(source, policyOverride = policy, options = {}) {
  const built = classify(source, policyOverride, options); const detectorRun = runDetectors(built.classification, createTreasuryDetectorRegistry(), { program: built.ir, analysis: built.analysis });
...
export function detectSources(sources, policyOverride = policy) {
  ...
  return runDetectors(classification, createTreasuryDetectorRegistry(), { program: ir, analysis });
}
