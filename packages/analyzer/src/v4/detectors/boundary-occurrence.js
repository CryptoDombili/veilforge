...
function keyFor(record) {
  return JSON.stringify([record.detector.detectorId, record.source.symbolId ?? record.source.sourceCandidateId,
    record.sink.sinkCandidateId, record.source.dataClass, record.source.callableId, record.sink.callableId,
...
export function selectBoundaryOccurrences(records, context) {
  ...
  for (const record of records) {
    ...
    if (falseExternalBoundary(record, context) || falseMetadataBoundary(record, context)) { falseBoundaryFilteredCount += 1; continue; }
