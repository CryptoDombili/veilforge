...
export async function runCli(argv = process.argv.slice(2), io = {}) {
  ...
  try {
    ...
    const options = await mergeExplicitConfig(parsed.options); let result;
    ...
    else result = await verifyExportCommand(parsed.positionals[0]);
    if (options.json) stdout(jsonDocument(result));
    else if (!options.quiet) stdout(parsed.command === 'scan' ? terminalSummary(result) : `${parsed.command}: verified\n`);
