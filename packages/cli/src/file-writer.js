...
export async function readExportDirectory(directory) {
  ...
  if (names.length !== EXPECTED.length || names.some((name, index) => name !== EXPECTED[index])) throw cliError('CLI_EXPORT_INVALID');
...
export async function writeExportPackage(pkg, directory, { overwrite = false } = {}) {
  ...
  if (exists) { const names = await readdir(output).catch(() => { throw cliError('CLI_OUTPUT_WRITE_FAILED'); }); if (names.some((name) => !EXPECTED.includes(name))) throw cliError('CLI_OUTPUT_WRITE_FAILED'); }
