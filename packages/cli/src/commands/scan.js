...
import { discoverProject } from '../source-discovery.js';
...
function positiveInteger(value, fallback, maximum = Number.MAX_SAFE_INTEGER) { if (value === undefined) return fallback; const number = Number(value); if (!Number.isInteger(number) || number <= 0 || number > maximum) throw cliError('CLI_ARGUMENT_INVALID'); return number; }
...
export async function scanCommand(options, io = {}) {
  ...
  const maxFileBytes = positiveInteger(options['max-file-bytes'], 2 * 1024 * 1024, 2 * 1024 * 1024); const maxProjectBytes = positiveInteger(options['max-project-bytes'], 20 * 1024 * 1024, 20 * 1024 * 1024);
  ...
  const requestedSettings = await jsonFile(options.settings, 'CLI_CONFIG_INVALID') ?? {};
  const project = await discoverProject({ source: options.source ?? [], file: options.file ?? [], cwd: io.cwd ?? process.cwd(), settings: requestedSettings, maxFileBytes, maxProjectBytes });
