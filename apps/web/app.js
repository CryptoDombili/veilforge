...
function renderShadow() {
  ...
  return workspaceHeader('Shadow Evidence Lab', 'Adversarial paths mapped from source evidence', 'Every campaign is deterministic and tied to a finding. This preview does not execute bytecode or claim full EVM emulation.', `<span class="status-chip ${lab.summary.mapped ? 'status-blocked' : 'status-ready'}">Defense ${lab.summary.defenseScore}/100</span>`) + `
    ${cinema}
...
function renderPrivacyTwin(report) {
  ...
  return workspaceHeader('Privacy Deployment Twin', 'Public Arc and APS readiness, side by side', 'A deterministic digital twin of the current source against Arc’s published privacy design.', `<span class="status-chip ${twin.readinessScore >= 90 ? 'status-ready' : 'status-review'}">Twin ${twin.readinessScore}/100</span>`) + `
    <div class="twin-roadmap"><div><span>${esc(twin.availability.label)}</span><strong>Honest preview, not a live APS deployment</strong><p>${esc(twin.availability.statement)}</p></div><code>${esc(shortHash(twin.twinId))}</code></div>
