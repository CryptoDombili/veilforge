...
export function proofSectionTemplate() {
  return `<section id="v4-proof" class="v4-secondary v4-proof panel" aria-labelledby="v4-proof-title">
    ...
    <div class="v4-proof-reconcile"><label for="v4-proof-reconcile-hash">Existing transaction hash</label><input id="v4-proof-reconcile-hash" type="text" inputmode="text" autocomplete="off" spellcheck="false" placeholder="0x…" maxlength="66"><button id="v4-proof-reconcile" type="button" disabled>Verify existing transaction</button></div>
...
export function renderTransactionSummary(summary) {
  ...
  return `<dl class="v4-proof-grid"><div><dt>Network</dt><dd>${esc(summary.networkName ?? 'Arc Testnet')}</dd></div><div><dt>From</dt><dd><code>${esc(short(summary.from))}</code></dd></div><div><dt>To trusted registry</dt><dd><code>${esc(short(summary.to))}</code></dd></div><div><dt>Registry contract</dt><dd>V${esc(summary.registryContractVersion ?? '—')}</dd></div><div><dt>Chain ID</dt><dd>${esc(summary.chainId)}</dd></div><div><dt>Report hash</dt><dd><code>${esc(short(summary.reportHash))}</code></dd></div><div><dt>Envelope / schema</dt><dd>${esc(summary.envelopeVersion ?? '—')} / ${esc(summary.schemaVersion ?? '—')}</dd></div><div><dt>Hash payload</dt><dd>${esc(summary.hashPayloadVersion ?? '—')}</dd></div><div><dt>Analysis</dt><dd>${summary.complete === false ? 'incomplete (acknowledged)' : 'complete'}${incompleteReasons}</dd></div><div><dt>Value</dt><dd>${esc(summary.value)}</dd></div><div><dt>Method</dt><dd>${esc(summary.registryMethod)}</dd></div><div><dt>Calldata</dt><dd><code>${esc(summary.calldataPreview ?? 'unavailable')}</code> · ${esc(summary.calldataBytes)} bytes</dd></div><div><dt>Calldata digest</dt><dd><code>${esc(short(summary.calldataDigest))}</code></dd></div><div><dt>Gas estimate</dt><dd>${esc(summary.gasEstimateStatus)}${summary.gasEstimate ? ` · ${esc(summary.gasEstimate)}` : ''}</dd></div><div><dt>Duplicate</dt><dd>${summary.duplicate ? 'publisher record present' : 'not found'} · ${esc(summary.duplicatePolicy)}</dd></div><div><dt>Explorer destination</dt><dd>${esc(summary.explorerExpectation ?? 'available after a validated transaction hash')}</dd></div></dl>`;
}
