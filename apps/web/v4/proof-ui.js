...
export function proofSectionTemplate() {
  return `<section id="v4-proof" class="v4-secondary v4-proof panel" aria-labelledby="v4-proof-title">
    ...
    <div class="v4-button-row"><button id="v4-proof-inspect-wallet" type="button" disabled>Inspect connected wallet</button><button id="v4-proof-preflight" type="button" disabled>Prepare transaction request</button></div>
    ...
    <p class="v4-proof-boundary">Transaction submission is disabled in Phase 5C-2. No wallet popup, signature, network switch, or send call is made.</p>
  </section>`;
}
...
export function renderTransactionSummary(summary) {
  if (!summary) return '';
  return `<dl class="v4-proof-grid"><div><dt>From</dt><dd><code>${esc(short(summary.from))}</code></dd></div><div><dt>To trusted registry</dt><dd><code>${esc(short(summary.to))}</code></dd></div><div><dt>Chain ID</dt><dd>${esc(summary.chainId)}</dd></div><div><dt>Value</dt><dd>${esc(summary.value)}</dd></div><div><dt>Method</dt><dd>${esc(summary.registryMethod)}</dd></div><div><dt>Calldata</dt><dd>${esc(summary.calldataBytes)} bytes (hidden)</dd></div><div><dt>Gas estimate</dt><dd>${esc(summary.gasEstimateStatus)}</dd></div><div><dt>Duplicate policy</dt><dd>${esc(summary.duplicatePolicy)}</dd></div></dl>`;
}
